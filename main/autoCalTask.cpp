...
#define MAXLEN 128
#define CHECKINTERVAL 10 // minutes

#define MAXCALDEV 1.0 // % deviation allowed before adjusting
		...
		if (lastminute != timeinfo.tm_min) {
			...
			if (minuteTmr == 0) {
				...
				if (calMssg.co2) { // then my calibrator is in the air...
					...
					dev = abs((100.0 * dev) / calMssg.co2); // absolute percent
					if (dev > MAXCALDEV) {
					...
					dev = abs((100.0 * dev)/calMssg.temperature); // absolute percent
					if ( dev  > MAXCALDEV) {
						userSettings.temperatureOffset = actualValues.temperature - calMssg.temperature;
						ESP_LOGI(TAG, "temperature sensor calibrated 1%2f %%", dev);
						save = true;
					}

					dev = actualValues.hum-calMssg.hum;  // checkRH
					dev = abs((100.0 * dev)/calMssg.hum); // absolute percent
					if ( dev  > MAXCALDEV) {
